...
export async function GET(req: NextRequest) {
  ...
  const linhas: { id: number; categoria: string; tipo_marcha: string | null; fase_julgamento: string | null }[] = atualData || []
  const pistas = [1, 2].map(id => {
    const linha = linhas.find(l => l.id === id)
    return { id, categoria: linha?.categoria || null, tipo_marcha: linha?.tipo_marcha || null, fase_julgamento: linha?.fase_julgamento || null }
  })
...
export async function PUT(req: NextRequest) {
  if (!autorizado(req)) return NextResponse.json({ error: 'Nao autorizado' }, { status: 401 })
  const { id, categoria, tipo_marcha, fase_julgamento } = await req.json()
